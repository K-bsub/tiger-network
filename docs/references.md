# References

## Tiger population and landscape

Jhala, Y.V., Qureshi, Q., Nayak, A.K., et al. (2023). *All India Tiger
Estimation Report 2022*. National Tiger Conservation Authority & Wildlife
Institute of India.

Jhala, Y.V., Qureshi, Q., & Gopal, R. (Eds.). (2008–2019). *Status of Tigers in
India* (2006, 2010, 2014, 2018 rounds). NTCA & WII.

Singh, B.V.R., & Sen, A. (2015). Comparative Analysis of Tiger Landscape
Complexes and Reserves in India. *Am Research Thoughts*, 1, 1796–1812.

Gopal, R., Qureshi, Q., Bhardwaj, M., Singh, R.K.J., & Jhala, Y.V. (2010).
Evaluating the status of the Endangered tiger *Panthera tigris* and its prey in
Panna Tiger Reserve, Madhya Pradesh, India. *Oryx*, 44(3), 383–389.
https://doi.org/10.1017/S0030605310000529 · *(Reserve-specific 2006 occupancy;
secondary context only — see docs/tbl_04_secondary_sources_2006_2010.md.)*

Jhala, Y.V., Qureshi, Q., & Gopal, R. (2011). Can the abundance of tigers be
assessed from their signs? *Journal of Applied Ecology*, 48, 14–24.
https://doi.org/10.1111/j.1365-2664.2010.01901.x · *(Method behind the 2010
double-sampling; source-population scale, not per-reserve.)*

Harihar, A., Pandav, B., MacMillan, D.C., et al. (2017). Defensible inference:
questioning global trends in tiger populations. *Conservation Letters*, 10(5),
502–505. · *(Documents non-comparability of 2006/2010 vs 2014+ estimates —
supports Decision 6.)*

Wikramanayake, E., Dinerstein, E., Seidensticker, J., et al. (2011). A
landscape-based conservation strategy to double the wild tiger population.
*Conservation Letters*, 4(3), 219–227.

## Forest and environment

Forest Survey of India. (2022). *India State of Forest Report 2021, Chapter 4*.
Ministry of Environment, Forest and Climate Change, Government of India.

Zanaga, D., et al. (2022). *ESA WorldCover 10 m 2021 v200*.
https://doi.org/10.5281/zenodo.7254221

Theobald, D.M., Oakleaf, J.R., Moncrieff, G., Voigt, M., Kiesecker, J., &
Kennedy, C.M. (2025). Global extent and change in human modification of
terrestrial ecosystems from 1990 to 2022. *Scientific Data*, 12, 606.
https://doi.org/10.1038/s41597-025-04892-2 · Data: 2022 static all-threats
layer, https://doi.org/10.5281/zenodo.14502573

NASA SRTM. (2013). *Shuttle Radar Topography Mission Global*. OpenTopography.
https://doi.org/10.5069/G9445JDF

AWS Open Data / Tilezen. *Terrain Tiles* (Mapzen synthesis DEM). Amazon Web
Services Open Data Registry. https://registry.opendata.aws/terrain-tiles/

## Spatial data

BirdLife International. *World Database of Key Biodiversity Areas*.
http://www.keybiodiversityareas.org/

GBIF.org (2026). *GBIF Occurrence Download: Panthera tigris in India*.
https://doi.org/10.15468/dl.npxmxx

GBIF.org (2026). *GBIF Occurrence Download: Mammalia target-group background,
India, 2006–2022*. https://doi.org/10.15468/dl.2d523e

OpenStreetMap contributors. *OpenStreetMap Data* (ODbL). Geofabrik:
https://download.geofabrik.de/asia/india.html

Natural Earth. *1:10m Cultural and Physical Vectors*.
https://www.naturalearthdata.com/

DataMeet India Community. (2011). *India District Boundaries* (CC BY 4.0).
https://github.com/datameet/maps

## Methods (R)

Pebesma, E. (2018). Simple Features for R: sf. *The R Journal*, 10(1), 439–446.

Hijmans, R.J. *terra: Spatial Data Analysis*. R package.

Phillips, S.J., Anderson, R.P., & Schapire, R.E. (2006). Maximum entropy
modeling of species geographic distributions. *Ecological Modelling*, 190,
231–259.

Gilardi, A., & Lovelace, R. *osmextract: Download and Import Open Street Map
Data Extracts*. R package. https://docs.ropensci.org/osmextract/

Hollister, J.W. *elevatr: Access Elevation Data from Various APIs*. R package.
https://github.com/USEPA/elevatr
