...
| **Status** | **Week 5 complete — census joined to boundaries; stats layer built.** `scripts/03_prepare_census.R` produced `stats_reserve_census_7755.gpkg` (58 reserves): wide within-reserve SECR (`pop_2014/2018/2022`), NTCA-notified `area_km2` (**Decision 8** — core+buffer total; ISFR-2021 Ch.4 is cross-check only), `density_2022` (52 values; NA for 5 flagged + Sundarbans), `baseline_year`, `census_status` (53 measured + 5 never-estimated). The area overwrite changed zero values (Week-3 placeholder already matched the notified totals). Config drift reconciled (Decision-6 census constants added; GBIF-download bounds kept separate). Prior: Week 4 — `census_reserve_long.csv` (148 within-reserve rows, 53 reserves; 2014/2018/2022; Decisions 6 + 7). Next: Week 6 — growth metrics (`change_abs`, `change_pct`, `aagr`) → `tbl_01_reserve_growth.csv`; **Decision 9** on zero-baseline handling. |
