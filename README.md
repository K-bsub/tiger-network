...
| **Status** | **Week 4 complete — all-reserve census time series extracted.** `census_reserve_long.csv` holds the within-reserve SECR series (148 rows, 53 reserves; 2014/2018/2022) + 5 never-estimated flags. **Decision 6** fixes the series to 2014/2018/2022 (2006/2010 have no per-reserve table — context only); **Decision 7** stores missing (reserve, round) cells as `NA`. Spot-check passed (2022 vs government release 17/17; 2014 sums to the report total). `area_km2` in the boundary layer is still the flagged placeholder. Prior: Week 3 — `boundary_reserves_all_7755.gpkg` (58 reserves, 55 geometry + 3 absent) from the NTCA DSS KML via a hand-reviewed crosswalk. Next: Week 5 — pivot the census wide, join to boundaries, overwrite `area_km2` from the census, compute density → `stats_reserve_census_7755.gpkg`. |
